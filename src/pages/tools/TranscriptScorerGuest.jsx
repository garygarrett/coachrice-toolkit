...
import { generateACCObservationReportPDF } from '../../lib/accObservationReport'

// Kept in sync with TranscriptScorer.jsx — update both if prompt changes
...
function GuestHeader() {
  ...
  return (
    <div style={gh.bar}>
      <img src={logo} alt="CoachRICE" style={gh.logo} />
      <span style={gh.title}>Transcript Reviewer</span>
      <button
        onClick={() => {
          sessionStorage.removeItem('coachrice_access')
...
export default function TranscriptScorerGuest() {
  ...
  useEffect(() => {
    const loadAutoTable = () => {
      if (window.jspdf?.jsPDF?.prototype?.autoTable) { setJsPdfLoaded(true); return }
      ...
      at.onload = () => setJsPdfLoaded(true)
      document.head.appendChild(at)
    }
    if (window.jspdf) { loadAutoTable(); return }
    const s = document.createElement('script')
    s.src = 'https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js'
    s.onload = loadAutoTable
    document.head.appendChild(s)
  }, [])
  ...
  const doDownloadPDF = () => {
    ...
    if (quals.length === 3) {
      ...
      observed.item2 = ep.coach_role_alignment || '';
      observed.item3 = '';
    }
    ...
    const codeToId = {
      ...
      'A6.1':'a6_1','A6.2':'a6_2','A6.3':'a6_3',
      'A7.1':'a7_1','A7.2':'a7_2','A7.3':'a7_3',
      'A8.1':'a8_1','A8.2':'a8_2','A8.3':'a8_3',
    };
    (evaluation.behavioral_statements || []).forEach(stmt => {
    ...
    const summaryStrengths = (evaluation.strengths || [])
      .map((s, i) => `${i + 1}. ${s.competency_name} (${s.code}): ${s.explanation}`)
      .join('\n\n');
    const summarySuggestions = (evaluation.suggestions || [])
      .map((s, i) => {
    ...
    const doc = generateACCObservationReportPDF({
      coachName: evaluation.coach_identifier || 'Submitted Coach',
      sessionDate: dateStr,
      observed,
      evidenceNotes,
      contraEvidenceNotes,
    ...
    doc.save('ACC_Feedback_' + sn + '.pdf');
  }
  // ── Wrapper ─────────────────────────────────────────────────────────────────
  const wrap = (children) => (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', fontFamily: "'Montserrat', sans-serif", background: COLORS['gray-light'] }}>
      <GuestHeader />
  ...
  if (stage === 'input') return wrap(
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '48px 32px', opacity: contentLoaded ? 1 : 0, transition: 'opacity 0.3s ease-in-out' }}>
      ...
      <div style={{ background: '#fff', border: `1px solid ${COLORS['gray-border']}`, borderRadius: '10px', padding: '24px', marginBottom: '24px' }}>
        <h3 style={{ fontSize: '14px', fontWeight: 700, color: COLORS.navy, margin: '0 0 16px' }}>SUBMISSION CONSENT</h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {[
            ...
            <label key={key} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', cursor: 'pointer' }}>
              <input
                type="checkbox"
                checked={consentChecked[key]}
                onChange={(e) => setConsentChecked(v => ({ ...v, [key]: e.target.checked }))}
                style={{ width: '18px', height: '18px', cursor: 'pointer', marginTop: '2px', flexShrink: 0 }}
              />
              <span style={{ fontSize: '13px', color: COLORS['text-main'], lineHeight: '1.5' }}>{text}</span>
            </label>
          ))}
        </div>
      </div>
      ...
      <div
        style={{ background: '#fff', border: `1px solid ${COLORS['gray-border']}`, borderRadius: '10px', padding: '40px', textAlign: 'center', marginBottom: '24px', cursor: 'pointer' }}
        onClick={() => fileInputRef.current?.click()}
        onDragOver={(e) => { e.preventDefault(); e.currentTarget.style.borderColor = '#69cce6' }}
        onDragLeave={(e) => { e.currentTarget.style.borderColor = COLORS['gray-border'] }}
        onDrop={(e) => {
          e.preventDefault()
      ...
      <textarea
        ...
        style={{ width: '100%', minHeight: '300px', padding: '16px', fontSize: '14px', fontFamily: 'monospace', border: `1px solid ${COLORS['gray-border']}`, borderRadius: '8px', marginBottom: '24px', boxSizing: 'border-box' }}
      />
      ...
      <button
        ...
        style={{ background: allConsented && transcript.trim() ? COLORS.navy : COLORS.gray, color: '#fff', border: 'none', padding: '12px 32px', fontSize: '14px', fontWeight: 600, borderRadius: '8px', cursor: allConsented && transcript.trim() ? 'pointer' : 'not-allowed', opacity: allConsented && transcript.trim() ? 1 : 0.6 }}
      >
        Get Feedback
      </button>
    </div>
  ...
  if (stage === 'running') return wrap(
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '120px 32px', textAlign: 'center' }}>
      ...
      <p style={{ fontSize: '15px', color: COLORS.gray, margin: 0 }}>
        This usually takes 30–60 seconds. We're reading through your transcript and evaluating your coaching against the ICF ACC competencies.
      </p>
    </div>
  )

  // ── REPORT STAGE ─────────────────────────────────────────────────────────────
  if (stage === 'report' && evaluation) return wrap(
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '32px', fontFamily: "'Montserrat', sans-serif" }}>
      <div style={{ display: 'flex', gap: '12px', marginBottom: '24px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: '240px' }}>
          <label style={{ fontSize: '12px', fontWeight: 600, color: COLORS.gray, letterSpacing: '0.5px' }}>DOWNLOAD NAME (optional)</label>
          <input
            type="text"
            value={downloadName}
            onChange={(e) => setDownloadName(e.target.value)}
            placeholder="Coach name or session identifier"
            style={{ width: '100%', padding: '10px 12px', fontSize: '14px', fontFamily: "'Montserrat', sans-serif", border: `1px solid ${COLORS['gray-border']}`, borderRadius: '6px', boxSizing: 'border-box', marginTop: '6px' }}
          />
        </div>
        <div style={{ display: 'flex', gap: '12px' }}>
          <button onClick={downloadText} style={{ background: 'transparent', color: COLORS.navy, border: `1px solid ${COLORS.navy}`, padding: '10px 16px', fontSize: '13px', fontWeight: 600, fontFamily: "'Montserrat', sans-serif", borderRadius: '6px', cursor: 'pointer' }}>
            📄 Download Text
          </button>
          <button onClick={downloadPDF} disabled={!jsPdfLoaded} style={{ background: jsPdfLoaded ? COLORS.navy : COLORS.gray, color: '#fff', border: 'none', padding: '10px 16px', fontSize: '13px', fontWeight: 600, fontFamily: "'Montserrat', sans-serif", borderRadius: '6px', cursor: jsPdfLoaded ? 'pointer' : 'not-allowed', opacity: jsPdfLoaded ? 1 : 0.5 }}>
                ...
                <div key={skill.code} style={{ background: '#f9fafc', borderRadius: '8px', border: `1px solid ${COLORS['gray-border']}`, padding: '16px', marginBottom: '12px' }}>
                  ...
                  {skill.evidence?.length > 0 && (
                    <div style={{ fontSize: '11px', color: '#555', marginTop: '8px', paddingTop: '8px', borderTop: `1px solid ${COLORS['gray-border']}` }}>
                      <strong>Evidence:</strong> {skill.evidence.map(e => `${e.timestamp}: "${e.quote}"`).join(' · ')}
                    </div>
                  )}
                ...
                {s.example_prompts?.length > 0 && (
                  <div style={{ fontSize: '12px', color: COLORS['text-main'] }}>
                    ...
                    <ul style={{ margin: '6px 0 0', paddingLeft: '20px' }}>
                      {s.example_prompts.map((prompt, j) => <li key={j} style={{ marginBottom: '3px', fontStyle: 'italic' }}>"{prompt}"</li>)}
                    </ul>
                  </div>
                )}
              </div>
      ...
      <button
        ...
        style={{ marginTop: '24px', background: 'none', border: `1px solid ${COLORS.navy}`, color: COLORS.navy, padding: '10px 16px', fontSize: '13px', fontWeight: 600, borderRadius: '6px', cursor: 'pointer' }}
      >
